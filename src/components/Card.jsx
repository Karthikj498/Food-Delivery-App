    ...
    const handleAddToCart = async () => {
        ...
        for (const item of data) {
            if (item.id == props.foodItem._id) {
                food.push(item); // Push the item into the food array
                break;
            }
        }
        if (food.length !== 0) {
            const selectedFood = food[0]; // Assuming you only want to consider the first item if there are multiple matches
            if (selectedFood.size === size) {
